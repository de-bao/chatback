 ...
 * - /api/* -> 转发到后端 FastAPI 服务器
 * - 其他路径 -> 转发到 GitHub Pages（前端）
 */

// 从环境变量读取配置（在 Cloudflare Dashboard 或 wrangler.toml 中配置）
// 后端 API 服务器地址（需要部署 FastAPI 到某个地方）
// 可以是：VPS、Railway、Render、Fly.io 等
const BACKEND_URL = env.BACKEND_URL || 'https://your-backend-server.com';

// GitHub Pages 地址（你的前端）
